...
function getActiveMenu($module = "", $action = "") {
  $body = getBody('get');
  if(!empty($body['module'])) {
    $moduleGet = $body['module'];
  }
  if(!empty($body['action'])) {
    $actionGet = $body['action'];
  }

  if(empty($module) && empty($moduleGet)) {
    echo 'active';
    return;
  }

  if(!empty($module) && !empty($moduleGet) && !empty($actionGet) && !empty($action)) { 
    if($module == $moduleGet && $action == $actionGet) {
      echo 'active';
      return;
    }
  }
