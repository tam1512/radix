...
function getActiveMenu($href, $isSub) {
  $currentLink = _WEB_HOST_ROOT.$_SERVER['REQUEST_URI'];

  if(!$isSub) {
    if($href != _WEB_HOST_ROOT) {
      if(strpos($currentLink, $href) !== false) {
        return 'class = "active"';
      }
    }
  } else {
...
function getMenu($dataMenu, $isSub = false) {
  if(!empty($dataMenu)) {
    ...
    foreach($dataMenu as $item) {
      echo '<li '.getActiveMenu($item['href'], $isSub).'><a href="'.$item['href'].'" target="'.$item['target'].'" title="'.$item['title'].'">'.$item['text'].'</a>';
