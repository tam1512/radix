...
function getCommentList($listData, $parentId, $id, $userInfor) {
  if(!empty($listData)) {
...
<div class="comment-list">
   <div class="head">
      <img
         src="<?php echo !empty($item['user_id']) && !empty($userInfor['avatar']) ? $userInfor['avartar'] : getAvatar($item['email']) ?>"
         alt="#">
   </div>
   <div class="body">
      <h4>
         <?php echo empty($item['user_id']) ? $item['fullname'] : $item['fullname']. ' <label class="badge badge-danger">'.getGroupByUserId($item['user_id']).'</lable>' ?>
      </h4>
      <div class="comment-info">
         <p><span><?php echo getDateFormat($item['create_at'], 'd/m/Y') ?> at<i
               ...
               href="<?php echo getLinkClient('blogs', 'detail', ['id' => $id,'comment_id' => $item['id']]).'#comment-form' ?>"><i
...
<?php
        unset($listData[$key]);
        getCommentList($listData, $item['id'], $id, $userInfor);
      }
    }
    echo '</div>';
  }
