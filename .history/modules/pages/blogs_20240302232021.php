<?php 
   ...
   if(empty($data)) {
      ...
      $listBlogOnPage = getRaw("SELECT blogs.id, blogs.title, thumbnail, view_count, users.fullname, blogs.create_at, blogs.user_id, users.email AS user_email, blogs.category_id AS cate_id, blog_categories.name AS cate_name FROM blogs INNER JOIN users ON blogs.user_id = users.id INNER JOIN blog_categories ON blog_categories.id = blogs.category_id ORDER BY blogs.create_at DESC LIMIT $offset, $blogOnPage");
   }
   $arrBlogShow = [];   
