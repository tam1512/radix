<?php 
   ...
   if(isPost()) {
      ...
      if(empty($errors)) {
         ...
         if($insertStatus) {
            ...
            setcookie('commentInfo', json_encode($commentInfor), time()+24*60*60*365);
   ...
   if(!empty($_COOKIE['commentInfor'])) {
      $commentInfor = $_COOKIE['commentInfor'];
   }
...
<div class="comments-form" id="comment-form">
   <h2 class="title">
      <?php echo !empty($commentCurrent['fullname']) ? 'Reply to '.$commentCurrent['fullname'].'<a class="text-danger" href="'.getLinkClient('blogs', 'detail', ['id' => $id]).'"><i class="fa fa-times"></i>Cancel</a>' : 'Leave a comment' ?>
   </h2>
