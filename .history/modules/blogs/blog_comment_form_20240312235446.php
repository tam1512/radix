<?php 
   ...
   if(isPost()) {
      $body = getBody('post');
      if(isset($body['submit-comment-form'])) {
         $errors = [];
         if(!empty($userInfor)) {
         ...
         if(empty($errors)) {
            ...
            if($insertStatus) {
               ...
               if(!empty($commentCurrent)) {
                  setFlashData('msg', 'Trả lời bình luận của '.$commentCurrent['fullname'].' thành công.');
                    ...
                    setFlashData('msg', 'Bình luận thành công.');
               } else {
            ...
            redirect(getLinkModule('blogs', $id).'#comment-form', true);
         } else {
...
<div class="comments-form" id="comment-form">
   <h2 class="title">
      <?php 
      if(!empty($userInfor)) {
         if(!empty($commentCurrent['fullname'])) {
            echo $userInfor['fullname']. '  <i class="fa fa-caret-right"></i>  '.$commentCurrent['fullname'].'<b> - <a href="'.getLinkAdmin('auth', 'logout').'">Đăng xuất</a>';
         } else {
      ...
      <div class="row">
         ...
         <div class="col-12">
            <div class="form-group button">
               <button type="submit" class="btn primary" name="submit-comment-form">Submit Comment</button>
            </div>
         </div>
      </div>
   </form>
