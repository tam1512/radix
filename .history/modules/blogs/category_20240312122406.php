<?php 
   ...
   $arrBlogs = getRaw('SELECT * FROM blogs WHERE category_id = ' . $id);
   $cateName = firstRaw('SELECT name FROM blog_categories WHERE id = '. $id)['name'];
   ...
   if(empty($data)) {
      ...
      //pagination
      $blogOnPage = getOption('page_blogs_quantity');
      ...
      $listBlogOnPage = getRaw("SELECT * FROM blogs WHERE category_id = $id LIMIT $offset, $blogOnPage");
   }
   $arrBlogShow = [];   
                  ...
                  <div class="single-blog">
                     ...
                     <div class="blog-bottom">
                        <div class="blog-inner">
                           <h4>
                              <a
                                 href="<?php echo getLinkModule('blogs', $item['id']) ?>"><?php echo $item['title'] ?></a>
                           </h4>
                           <?php echo html_entity_decode($item['description']) ?>
                           <div class="meta">
                              <span><i class="fa fa-bolt"></i>
                                 <a href="<?php echo getLinkModule('blog_categories', $id) ?>">
                                    <?php echo $cateName ?>
                                 </a>
                              </span>
                              <span><i
                  ...
                  <li class="prev">
                     <a href="
                        <?php 
                           ...
                           echo getLinkModule('blog_categories', $id, '', '', ['page'=>$prevPage]);
                        ?>">
                  ...
                  <?php 
                     if(!empty($numPage)) {
                        ...
                        if($numPage <= $limitPagination) {
                           for($i = 1; $i <= $numPage; $i++) {
                              if($page == $i) {
                                 echo '<li class="active"><a href="'.getLinkModule('blog_categories', $id, '', '', ['page'=>$i]).'">'.$i.'</a></li>';
                              } else {
                                 echo '<li><a href="'.getLinkModule('blog_categories', $id, '', '', ['page'=>$i]).'">'.$i.'</a></li>';
                              }
                           }
                        } else {
                           for($i = $begin; $i <= $end; $i++) {
                  ...
                  <li class="next">
                     <a href="
                        <?php
                           ...
                           echo getLinkModule('blog_categories', $id, '', '', ['page'=>$nextPage]);
                        ?>">
