                              ...
                              <span><i class="fa fa-bolt"></i>
                                 <?php 
                                    foreach($listCategories as $cate) {
                                       if($cate['id'] == $item['auth_id']) {
                                          echo '<a href="'.getLinkClient('blogs', 'auth', ["id" => $cate['id']]).'">'.$cate['name'].'</a>';
                                          break;
                                       }
                                    }
                                 ?>
                  ...
                  <li class="prev">
                     <a href="
                        <?php 
                           ...
                           echo getLinkClient('blogs', 'auth', ['id'=>$id, 'page'=>$prevPage]);
                        ?>">
                  ...
                  <?php 
                     if(!empty($numPage)) {
                        ...
                        if($numPage <= $limitPagination) {
                           for($i = 1; $i <= $numPage; $i++) {
                              if($page == $i) {
                                 echo '<li class="active"><a href="'.getLinkClient('blogs', 'auth', ['id' => $id, 'page' => $i]).'">'.$i.'</a></li>';
                              } else {
                                 echo '<li><a href="'.getLinkClient('blogs', 'auth', ['id' => $id, 'page' => $i]).'">'.$i.'</a></li>';
                              }
                           }
                        } else {
                           for($i = $begin; $i <= $end; $i++) {
                  ...
                  <li class="next">
                     <a href="
                        <?php
                           ...
                           echo getLinkClient('blogs', 'auth', ['id' => $id, 'page' => $nextPage]);
                        ?>">
