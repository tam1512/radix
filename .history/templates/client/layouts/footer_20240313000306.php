...
if(isPost()) {
   $body = getBody('post');
   if(isset($body['btn-subscribe'])) {
      $errors = [];
      $fullname = trim($body['fullname-subscribe']);
      $email = trim($body['email-subscribe']);
   
      if(empty($fullname)) {
         $errors['fullname-subscribe']['required'] = 'Không được để trống họ tên';
      }
   
      if(empty($email)) {
         $errors['email-subscribe']['required'] = 'Không được để trống họ tên';
      } else if (!isEmail($email)) {
         $errors['email-subscribe']['invalid'] = 'Email không đúng định dạng';
      }
      ...
      if(empty($errors)) {
         ...
         if($insertStatus) {
            ...
            setFlashData('msg-subscribe', 'Đăng ký thành công');
            setFlashData('msg_type-subscribe', 'success');
         } else {
            setFlashData('msg-subscribe', 'Đăng ký thất bại');
            setFlashData('msg_type-subscribe', 'danger');
         }
         redirect(trim($_SERVER['PATH_INFO'],'/').'#form-subscribe');
      } else {
         setFlashData('msg-subscribe', 'Dữ liệu không hợp lệ!');
         setFlashData('msg_type-subscribe', 'danger');
         setFlashData('errors-subscribe', $errors);
         setFlashData('old-subscribe', $body);
         redirect(trim($_SERVER['PATH_INFO'],'/').'#form-subscribe');
      }
   }
}
...
$message = getFlashData('msg-subscribe');
$msgType = getFlashData('msg_type-subscribe');
$errors = getFlashData('errors-subscribe');
$old = getFlashData('old-subscribe');
