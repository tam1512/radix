...
$listPortfolioOnPage = getRaw("SELECT portfolios.id, portfolios.name, thumbnail, users.fullname, portfolios.create_at, portfolios.user_id, users.email AS user_email FROM portfolios INNER JOIN users ON portfolios.user_id = users.id $filter ORDER BY portfolios.create_at DESC LIMIT $offset, $serviceOnPage");
