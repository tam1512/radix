<?php 
   ...
   $listAllUsers = getRaw("SELECT id, fullname, email FROM userss ORDER BY create_at DESC");
...
// Số lượng phân trang
$numPage = ceil($countRowportfolios/$serviceOnPage);
...
$listPortfolioOnPage = getRaw("SELECT portfolios.id, portfolios.name, thumbnail, users.fullname, portfolios.create_at, portfolios.user_id, users.email AS user_email, portfolios.category_id AS cate_id, portfolio_categories.name AS cate_name FROM portfolios INNER JOIN users ON portfolios.user_id = users.id INNER JOIN portfolio_categories ON portfolio_categories.id = portfolios.category_id $filter ORDER BY portfolios.create_at DESC LIMIT $offset, $serviceOnPage");
