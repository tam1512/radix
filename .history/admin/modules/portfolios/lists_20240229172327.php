...
if(!empty($cateId)) {
   ...
   if(empty($userId) && empty($keyword)) {
      ...
      foreach($listAllPortFolios as $item) {
         if(preg_match('/^.*.'.$item['name'].'.*$/i', $keywork)) {
            foreach($item['categories'] as $cate) {
               if($cate == $cateId) {
                  $countRowPortfolios++;
                  break;
         ...
         <tbody>
            <?php 
               if(!empty($listPortfolioOnPage)):
                  $count = 0;
                  for($i = $offset; $i < $offset+$portfolioOnPage; $i++):
                     $count++;
                     echo '<pre>';
                     print_r($listPortfolioOnPage[$i]);
                     echo '</pre>';
                     die();
            ?>
            <tr>
               ...
               <td>
                  <?php 
                     foreach($listPortfolioOnPage[$i]['categories'] as $cate):
                  ?>
                  <p class="alert alert-success"><?php echo $cate ?></p>
                  <?php
                     endforeach;
                  ?>
               </td>
