...
$groupId = getGroupId();
$group = firstRaw("SELECT root FROM users WHERE id = $groupId");
...
$listAllusers = getRaw("SELECT id, name FROM users");
// Xử lý tìm kiếm
$filter = '';
if(isGet()) {
   ...
   if(!empty(getBody()["users"])) {
      ...
      $filter .= " $operator group_id = $users";
   }
            ...
            <select name="users" class="form-control">
               ...
               <?php 
                  if(!empty($listAllusers)):
                     foreach($listAllusers as $group):
               ?>
               <option value="<?php echo $group['id'] ?>"
                  <?php echo (!empty($users) && $users==$group['id'])? 'selected' : false ?>>
