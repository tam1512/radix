...
$listcontactOnPage = getRaw("SELECT contacts.id, contacts.title, thumbnail, view_count, users.fullname, contacts.create_at, contacts.user_id, users.email AS user_email, contacts.category_id AS cate_id, departments.name AS cate_name FROM contacts INNER JOIN users ON contacts.user_id = users.id INNER JOIN departments ON departments.id = contacts.category_id $filter ORDER BY contacts.create_at DESC LIMIT $offset, $contactOnPage");
                                 ...
                                 <tr>
                                    ...
                                    <td class="text-center">
                                       <a class="btn btn-success"
                                          href="<?php echo !empty($keyword) ? getLinkAdmin('contacts', '', ['keyword'=>$keyword, 'user_id'=>$user["id"], 'cate_id'=>$departmentId]) : getLinkAdmin('contacts', '', ['user_id'=>$user["id"], 'cate_id'=>$departmentId]) ?>">Chọn</a>
                                    </td>
                                 </tr>
                                 <?php 
                                    endforeach; else:
               ...
               <div class="form-group d-flex mw-210">
                  <select name="select" id="select" class="form-control" readonly disabled>
                     ...
                     <option value="<?php echo $cate['id'] ?>"
                        <?php echo (!empty($departmentId) && $cate['id'] == $departmentId) ? 'selected' : false ?>>
                        <?php echo $cate['name'] ?>
                     </option>
                     <?php 
                           endforeach;
