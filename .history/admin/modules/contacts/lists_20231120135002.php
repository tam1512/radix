...
$listContactOnPage = getRaw("SELECT contacts.id, contacts.fullname, contacts.email, contacts.create_at, contacts.deparment_id AS department_id, departments.name AS cate_name FROM contacts INNER JOIN users ON contacts.user_id = users.id INNER JOIN departments ON departments.id = contacts.deparment_id $filter ORDER BY contacts.create_at DESC LIMIT $offset, $contactOnPage");
