...
$listContactOnPage = getRaw("SELECT contacts.id, contacts.fullname, contacts.email, contacts.message, contacts.status, contacts.create_at, contacts.department_id, departments.name AS department_name FROM contacts INNER JOIN departments ON departments.id = contacts.department_id $filter ORDER BY contacts.create_at DESC LIMIT $offset, $contactOnPage");


// echo handleQueryString('contacts', $page);
echo '<pre>';
print_r(handleQueryString('contacts', $page));
echo '</pre>';
