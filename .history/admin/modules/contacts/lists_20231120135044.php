...
$listContactOnPage = getRaw("SELECT contacts.id, contacts.fullname, contacts.email, contacts.create_at, contacts.deparment_id departments.name AS department_name FROM contacts INNER JOIN departments ON departments.id = contacts.deparment_id $filter ORDER BY contacts.create_at DESC LIMIT $offset, $contactOnPage");
