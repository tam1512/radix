...
$listBlogOnPage = getRaw("SELECT contacts.id, contacts.title, thumbnail, view_count, users.fullname, contacts.create_at, contacts.user_id, users.email AS user_email, contacts.category_id AS cate_id, departments.name AS cate_name FROM contacts INNER JOIN users ON contacts.user_id = users.id INNER JOIN departments ON departments.id = contacts.category_id $filter ORDER BY contacts.create_at DESC LIMIT $offset, $blogOnPage");
