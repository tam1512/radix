      ...
      <table class="table table-bordered">
         ...
         <tbody>
            ...
            <tr>
               ...
               <td>
                  <?php
                   echo isIcon($contact['thumbnail']) ? $contact['thumbnail'] : '<img src="'.$contact['thumbnail'].'" alt="thumbnail" width="80%">' 
                   ?>
               </td>
               <td>
                  <a
                     href="<?php echo getLinkAdmin('contacts', 'edit', ['id'=>$contact['id']]) ?>"><?php echo $contact['title'] ?></a>
                  <a href="<?php echo getLinkAdmin('contacts', 'duplicate', ['id' => $contact['id']]) ?>"
                     class="btn btn-danger btn-sm btn-duplicate ml-2">Nhân bản</a>
                  <span class="btn btn-success btn-sm btn-duplicate">Lượt xem :
                     <?php echo $contact["view_count"] ?></span>
                  <a href="<?php echo getLinkAdmin('contacts', 'seen', ['id' => $contact['id']]) ?>"
                     class="btn btn-primary btn-sm btn-duplicate">Xem</a>
               </td>
               <td><a
                     href="<?php echo getLinkAdmin('contacts', "", ['user_id'=>$contact['user_id']])?>"><?php echo $contact['fullname']  ?></a>
               </td>
               <td><a
                     href="<?php echo getLinkAdmin('contacts', "", ['department_id'=>$contact['department_id']])?>"><?php echo $contact['cate_name']  ?></a>
               </td>
