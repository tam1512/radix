               ...
               <td class="d-flex flex-column justify-content-between align-items-center">
                  <?php 
                     foreach($listStatus as $key=>$value) {
                           ...
                           if($isStatus) {
                              echo $key == 0 ? '<a class="text-info" href="'.getLinkAdmin('comments', 'status', ['id' => $comment['id'], 'status' => $key]).'">Duyệt</a>' : '<a class="text-danger mt-2" href="'.getLinkAdmin('comments', 'status', ['id' => $comment['id'], 'status' => $key]).'">Hủy duyệt</a>';
                           }
                        }
                     }
                  ?>
