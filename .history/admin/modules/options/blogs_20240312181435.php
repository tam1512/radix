<?php 
  ...
  if($isRoot) {
    ...
    $checkPermission = checkPermission($permissionData, 'options', 'blogs');
  }
  
  
  if(!$checkPermission) {
    setFlashData('msg', 'Bạn không có quyền truy cập vào trang Thiết lập bài viết');
   ...
   if(isPost()) {
      ...
      $quantityBlogsOnPage = trim($body['page_blogs_quantity']);
      ...
      if(empty($quantityBlogsOnPage)) {
         $errors['page_blogs_quantity']['required'] = 'Không được để trống số lượng bài viết trên trang';
      } else if(!isNumberInt($quantityBlogsOnPage)) {
         $errors['page_blogs_quantity']['invalid'] = 'Số lượng bài viết trên trang phải là số';
      }
               ...
               <div class="flex-grow-1 align-self-center overflow-hidden">
                  <div class="row">
                     ...
                     <div class="col-12">
                        <div class="form-group">
                           <label
                              for="page_blogs_quantity"><?php echo getOption('page_blogs_quantity', 'label') ?></label>
                           <input type="text" name="page_blogs_quantity" id="page_blogs_quantity" class="form-control"
                              placeholder="<?php echo getOption('page_blogs_quantity', 'label') ?>..."
                              value="<?php echo !empty($old) ? $old['page_blogs_quantity'] : getOption('page_blogs_quantity') ?>">
                           <?php echo !empty($errors['page_blogs_quantity']) ? form_error('page_blogs_quantity', $errors, '<span class="error">', '</span>') : false ?>
                        </div>
                     </div>
                  </div>
               </div>
