...
$listDeContactTypesOnPage = getRaw("SELECT id, name, create_at, user_id, (SELECT count(contacts.id) FROM contacts WHERE contacts.type_id = contact_types.id) as contacts_count FROM contact_types $filter ORDER BY create_at DESC LIMIT $offset, $typeOnPage");
